                                ...
                                @if ($item->status == 'recharge')
                                    @if (!$item->recharge_status)
                                        ...
                                        <a href="{{ route('employee.receiveUnusedProductStatusUpdate', ['item' => $item, 'status' => 'recharge_status', 'update' => 'bad']) }}"
                                            ...
                                            onclick="return confirm('Are you sure? This product is bad?');">Not Possible</a>
                                ...
                                @if ($item->status == 'repair')
                                    @if (!$item->repair_status)
                                        <button type="button" class="btn btn-primary" data-toggle="modal"
                                            data-target="#reuse_{{ $item->id }}" data-whatever="@mdo">Reuse</button>
                                        <button type="button" class="btn btn-primary" data-toggle="modal"
                                            data-target="#not_possible_{{ $item->id }}" data-whatever="@mdo">Not
                                            Possible</button>
                                        <a href="{{ route('employee.receiveUnusedProductStatusUpdate', ['item' => $item, 'status' => 'repair_status', 'update' => 'reuse']) }}"
                                            class="btn btn-success btn-xs"
                                            onclick="return confirm('Are you sure? You to Reuse?');">Reuse</a>
                                        <a href="{{ route('employee.receiveUnusedProductStatusUpdate', ['item' => $item, 'status' => 'repair_status', 'update' => 'bad']) }}"
                                            class="btn btn-danger btn-xs"
                                            onclick="return confirm('Are you sure? This product is bad?');">Not Possible</a>
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        @if ($item->status == 'recharge')
                                            <form
                                                accept="{{ route('employee.receiveUnusedProductStatusUpdate', ['item' => $item, 'status' => 'recharge_status', 'update' => 'bad']) }}"
                                                method="POST">
                                            @elseif ($item->status == 'repair')
                                                <form
                                                    accept="{{ route('employee.receiveUnusedProductStatusUpdate', ['item' => $item, 'status' => 'repair_status', 'update' => 'bad']) }}"
                                                    method="POST">
                                        @endif
                                        @csrf
                                        <div class="form-group">
                                            <label for="recipient-name" class="col-form-label">Bad:</label>
...
@push('js')
    <script>
        $(document).on('input', '.quantity', function(e) {
            ...
            var due_qty = that.closest('.modal-body').find('.due_quantity').val();
            if (quantity > due_qty) {
