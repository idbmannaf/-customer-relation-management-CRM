    ...
    <div class="card shadow">
        <div class="card-header bg-info">
            <div class="card-title">Visits Of {{ $visit_plan->date_time }}
                @can('visit-add')
                    <a class="btn btn-danger btn-sm" href="{{ route('admin.visit.add', $visit_plan) }}">Visit Report</a>
                @endcan

                @if ($visit_plan->call)
                    <a href="" class="btn btn-success btn-xs" data-toggle="modal" data-target="#vp-{{ $visit_plan->id }}"
                        data-whatever="@fat">Service Call Details</a>
                    @if ($visit_plan->customer_office_location_id)
                        <a href="javascript:void(0)" class="btn btn-danger">Already Checked</a>
                    @else
                        <a href="{{ route('admin.customerVisited', $visit_plan) }}" class="btn btn-light btn-xs"
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            @if ($visit->status == 'pending')
                                                @can('visit-plan-update')
                                                    <a class="dropdown-item"
                                                        href="{{ route('admin.customerVisitEdit', ['visit_plan' => $visit_plan, 'visit' => $visit]) }}">Edit</a>
                                                @endcan
                                            @endif
                                            <a class="dropdown-item"
                                                href="{{ route('admin.customerVisitview', ['visit_plan' => $visit_plan, 'visit' => $visit]) }}">Details</a>

                                            @can('visit-plan-status-update')
                                                @if ($visit->status == 'pending')
                                                    <a class="dropdown-item"
                                                        href="{{ route('admin.customerVisitStatusUpdate', ['visit_plan' => $visit_plan, 'visit' => $visit, 'status' => 'approved']) }}"
                                                        onclick="return confirm('Are you Sure? you want to Approved this Visit?')">Approved
                                                    </a>
                                                    <a class="dropdown-item"
                                                        href="{{ route('admin.customerVisitStatusUpdate', ['visit_plan' => $visit_plan, 'visit' => $visit, 'status' => 'rejected']) }}"
                                    ...
                                    {{-- <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            <a class="dropdown-item" href="{{ route('admin.visit.edit', $visit) }}">Edit</a>

                                            <a class="dropdown-item"
                                                href="{{ route('admin.visitUpdateStatus', ['visit' => $visit, 'status' => 'approved']) }}"
                                                onclick="return confirm('Are you Sure? you want to Approv this Visit?')">Approved
                                            </a>

                                            <a class="dropdown-item"
                                                href="{{ route('admin.visitUpdateStatus', ['visit' => $visit, 'status' => 'rejected']) }}"
                                                onclick="return confirm('Are you Sure? you want to Reject this Visit?')">Rejected
                                            </a>
                                    ...
                                    @if (($visit->status == 'approved' || $visit->status == 'completed') && !$visit_plan->invoice_id)
                                        @if (!$visit->offer_id && $visit_plan->service_type != 'collection')
                                            <a class="btn btn-primary btn-sm"
                                                href="{{ route('admin.customerOffer.create', ['customer' => $visit_plan->customer, 'visit' => $visit]) }}">
                                                Make
                                                Offer/Quotation</a>
