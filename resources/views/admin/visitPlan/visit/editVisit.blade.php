...
@section('content')
    <div class="card shadow">
        ...
        <div class="card-body">
            <form action="{{ route('admin.customerVisitUpdate', ['visit_plan' => $visit_plan, 'visit' => $visit]) }}"
                method="POST">
                @csrf
                ...
                <div class="card">
                    ...
                    @if (has_in_array($visit_plan->service_type, 'service'))
                        @include('admin.visitPlan.visit.part.edit.service')
                    @endif
                    @if (has_in_array($visit_plan->service_type, 'sales'))
                        {{-- @include('admin.visitPlan.visit.part.edit.sales') --}}
                    @endif
                    @if (has_in_array($visit_plan->service_type, 'collection'))
                        @include('admin.visitPlan.visit.part.edit.collection')
                    @endif
                    @if (has_in_array($visit_plan->service_type, 'delivery'))
                        @include('admin.visitPlan.visit.part.edit.delivery')
                    @endif
                    ...
                    <div class="row">
                        ...
                        <div class="remarks pt-2 col-12">
                            <div class="">
                                ...
                                <input type="file" multiple id="file" name="file[]" data-url="{{ route('global.fileStore', ['type' => 'visit','visit'=>$visit->id]) }}">
                            </div>
                            <div class="images">
                                @foreach ($visit->files as $attachment)
                                <div class="attachment">
                                    <span class="serial"></span><a class="badge badge-success" href="{{ route('imagecache', [ 'template'=>'original','filename' => $attachment->name ]) }}"> {{$attachment->name}}</a> <a href="{{route('global.fileDelete',['file'=>$attachment->id])}}"  class="badge badge-danger deleteAttachment"><i class="fas fa-times"></i></a>
                                </div>
