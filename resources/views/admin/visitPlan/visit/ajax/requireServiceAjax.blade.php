...
<tr class="serReqSerial">
    ...
    <td class="showProducts">
        <select id="service_product_{{$serviceRequirement->id}}" onchange="updateServiceRequire(this,'product_id')" name="product_2" class="form-control user-select service_product"
            ...
            data-ajax-url="{{ route('global.productAllAjax',['type'=>$serviceRequirement->product_type]) }}"
 ...
 --}}
        ...
        <input type="hidden" class="updateServiceRequirement" value="{{route('admin.updateRequirementsOfBattAndSpearPartAjax', ['visit_plan' => $visit_plan,'item'=>$serviceRequirement->id])}}">
    </td>
    ...
    <td><a href="{{ route('admin.deleteRequirementsOfBattAndSpearPartAjax', ['visit_plan' => $visit_plan,'item'=>$serviceRequirement->id]) }}" class="btn btn-sm btn-danger DeleteServiceReq"><i class="fas fa-trash"></i></a></td>
</tr>
@push('js')
<script>
    $(document).on('change', '.spear_part_cat2', function() {
        ...
        if (that.val()) {
            if (that.val() == 'spare_parts') {
                var html = ` <select id="service_product_{{$serviceRequirement->id}}" onchange="updateServiceRequired(this,'product_id')" name="product_2" class="form-control user-select service_product" data-55-url="{{route('admin.updateRequirementsOfBattAndSpearPartAjax', ['visit_plan' => $visit_plan,'item'=>$serviceRequirement->id])}}"
            data-placeholder="Product Name / Model"
