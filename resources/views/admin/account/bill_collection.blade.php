                        ...
                        <tr>
                            ...
                            <td>
                                @if (!$item->hasVisitPlan())
                                    <a href="{{ route('admin.assignToBillCollection', ['invoice' => $item]) }}"
                                        class="btn btn-success btn-sm">Assign to Collection</a>
                                @else
                                @endif
                                @if ($item->hasTransectionHistory())
                                    <a href="{{ route('admin.transectionHistory', ['invoice' => $item]) }}"
                                        class="btn btn-warning btn-sm">Transection</a>
                                @endif
                                @if ($item->hasVisitPlan())
                                    <a href="{{ route('admin.customerVisitPlansOfInvoice', ['invoice' => $item]) }}"
