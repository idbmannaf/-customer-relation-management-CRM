@extends('admin.layouts.adminMaster')
@push('title')
    | Admin Dashboard | Attendance Report
@endpush

@push('css')
    <link href="{{ asset('https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/css/select2.min.css') }}"
        rel="stylesheet" />
    <link rel="stylesheet"
        href="{{ asset('https://cdn.jsdelivr.net/npm/@ttskch/select2-bootstrap4-theme@x.x.x/dist/select2-bootstrap4.min.css') }}">
@endpush
@section('content')
    <div class="card shadow">
        <div class="card-header bg-info">
            <div class="card-title">Attendance Report
                {{-- <a href="{{ route('admin.company.create') }}" class="btn btn-danger"><i
                        class="fas fa-plus"></i></a> --}}
            </div>
        </div>
        @include('alerts.alerts')
        <div class="card-body">
            <form action="{{ route('admin.companyWiseAttendanceReport', ['type'=>'filter']) }}" method="GET">
                <div class="row py-2">
                    <div class="col-12 col-md-3">

                    </div>
                    <div class="col-12 col-md-3">
                        <input type="date" name="date" value="{{ $input ? $input['date'] : old('date') }}" required
                            class="form-control">
                    </div>

                    <div class="col-12 col-md-3">
                        {{-- <label for="">Company:</label> --}}
                        <select id="user" name="company"
                            class="form-control select2-container step2-select select2 @error('employee') is-invalid @enderror"
                            data-placeholder=" Company  Name">
                            @foreach ($companies as $company)
                            <option value="{{$company->id}}"  {{isset($input['company']) && $input['company'] == $company->id ? 'selected' : ''}}>{{$company->name}}</option>
                            @endforeach

                        </select>
                    </div>
                    <div class="col-12 col-md-3">
                        {{-- <div>&nbsp;</div> --}}
                        <input type="submit" class="btn btn-info" value="Submit">
                    </div>
                </div>
            </form>

                @if (isset($input['date']) && isset($input['company']))
                    @include('admin.attendance.ajax.catwiseAttendanceList')
                @endif

        </div>
    </div>
@endsection



@push('js')
    <script src="{{ asset('https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/js/select2.min.js') }}"></script>

    <script>
        $(document).ready(function() {
            $('.select2').select2({
                theme: 'bootstrap4',
               });
        });
    </script>
@endpush
