...
@section('content')
    <div class="card shadow">
        ...
        <div class="card-body">
            ...
            <form action="{{ route('admin.location.update',['location'=>$officeLocation]) }}" method="POST" enctype="multipart/form-data">
                @method('PATCH')
                @csrf
                <div class="row">
                    <div class="col-12 col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="title" class="form-control @error('title') is-invalid @enderror "
                                id="title" placeholder="Location Name Here" value="{{ $officeLocation->title }}">
                        </div>
                        ...
                        <div class="form-group">
                            <div class="col-md-12 my-2">
                                ...
                                <input type="text" name="location"
                                    ...
                                    value="{{ $officeLocation->google_location }}">
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="lat" class="form-control @error('lat') is-invalid @enderror " id="lat"
                                placeholder="Latitude Here" value="{{ $officeLocation->lat }}">
                        </div>

                        <div class="form-group">
                            <label for="lng">Longitude</label>
                            <input type="text" name="lng" class="form-control @error('lng') is-invalid @enderror " id="lng"
                                placeholder="Longitude Here" value="{{ $officeLocation->lng }}">
                        </div>
                        <div class="form-group">
                            <label for="office_start_time">Office Start Time</label>
                            <input type="time" name="office_start_time" class="form-control  @error('office_start_time') is-invalid @enderror " id="lng" placeholder="Start Time  Here"  value="{{$officeLocation->office_start_time}}">
                        </div>
                        <div class="form-group">
                            <label for="office_end_time">Office End Time</label>
                            <input type="time" name="office_end_time" class="form-control  @error('office_end_time') is-invalid @enderror  " id="lng" placeholder="end Time  Here" value="{{$officeLocation->office_end_time}}">
                        </div>
                        <div class="form-group">
                            <label for="office_late_time">Office Late Time</label>
                            <input type="time" name="office_late_time"
                                class="form-control  @error('office_late_time') is-invalid @enderror  " id="lng"
                                placeholder="Late Time  Here" value="{{$officeLocation->office_late_time}}">
                        </div>
                        <div class="form-group">
                            <label for="office_absent_time">Office Absent Time</label>
                            <input type="time" name="office_absent_time"
                                class="form-control  @error('office_absent_time') is-invalid @enderror  " id="lng"
                                placeholder="Absent Time  Here" value="{{$officeLocation->office_absent_time}}">
                        </div>
                    </div>
                    <div class="col-12 col-md-6">
                        <div class="form-group">
                            ...
                            <select name="division" id="division"
                                ...
                                @foreach ($divisions as $division)
                                    <option {{ $officeLocation->division_id == $division->id ? 'selected' : '' }}
                        ...
                        <div class="form-group">
                            ...
                            <select name="district" id="district"
                                ...
                                <option {{$officeLocation->district_id == $district->id ? 'selected': ''}} value="{{ $district->id }}">{{ $district->name }}
                                @endforeach
                        ...
                        <div class="form-group">
                            ...
                            <select name="thana" id="thana"
                                ...
                                <option {{$officeLocation->thana_id == $thana->id ? 'selected': ''}} value="{{ $thana->id }}">{{ $thana->name }}
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="company" id="company"
                                ...
                                @foreach ($companies as $company)
                                    <option {{ $company->id == $officeLocation->company_id ? 'selected' :'' }} value="{{ $company->id }}">{{ $company->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group ">
                            ...
                            @if ($officeLocation->featured_image)
                                <img
                                    src="{{ route('imagecache', ['template' => 'sbixs', 'filename' => $officeLocation->fi()]) }}" />
                                <br>
                            @endif
                        </div>
                        <div class="form-group ">
                            <label for="active"><input type="checkbox" {{$officeLocation->active ? 'checked' : ''}} name="active" id="active"> Active</label>
                        </div>
                        <div class="form-group ">
