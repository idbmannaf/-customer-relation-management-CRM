        ...
        <div class="row">
            <div class="col-12 col-md-6 pb-2">
                ...
                <select name="payment_method" id="payment_method" class="form-control">
                    ...
                    <option {{$visit->payment_method == 'cash' ? 'selected' :  ''}} value="cash">Cash</option>
                    <option {{$visit->payment_method == 'cheque' ? 'selected' :  ''}} value="cheque">Cheque</option>
                </select>
            </div>
            <div class="col-12 col-md-6 payment_collection_date">
                @if ($visit->payment_method == 'cash')
                ...
                <input type="date"
                    ...
                    value="{{$visit->payment_collection_date}}">
                @endif

            </div>
            <div class="col-12 col-md-6 show_payment_maturity_date">
                @if ($visit->payment_method == 'cheque')
            ...
            @if ($invoice)
                <div class="col-12">
                    <label for="collection_amount">Collection Amount @if ($visit_plan->payment_maturity_date)  {{$visit_plan->payment_collection_amount}} @else {{ $invoice->due > 0 ? $invoice->due : $invoice->total_amount }} @endif</label>
                    <input type="number"
                        class="form-control collection_amount @error('collection_amount') is-invalid @enderror "
                        name="collection_amount" value="{{$visit->collection_amount}}">
                </div>
                <div class="col-12 showNextDate">
                    <label for="next_collection_date">Next Collection Date <b id="due">Due:{{  ($invoice->due > 0 ? $invoice->due : $invoice->total_amount) - $visit->collection_amount }}</b></label>
                    <input type="date"
                        class="form-control  @error('next_collection_date') is-invalid @enderror "
                        name="next_collection_date" value="{{ \Carbon\Carbon::parse($visit->next_paymnet_collection_date)->format('Y-m-d') }}">
                </div>
{{--
                <div class="col-12">
                    <label for="collection_amount">Collection Amount
                        ({{ $invoice->due > 0 ? $invoice->due : $invoice->total_amount }})</label>
                    <input type="number"
                        class="form-control collection_amount @error('collection_amount') is-invalid @enderror "
