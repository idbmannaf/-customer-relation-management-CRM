    ...
    <div class="card shadow">
        <div class="card-header bg-info">
            <div class="card-title">Service Call Wise Assign To (Visit Plans)
                @if ($employee->team_admin && $employee->company->access_all_call && !$call->done_by)
                    @if (request()->inhouse)
                            ...
                            <tr>
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            ...
                                            <a class="dropdown-item"
                                                href="{{ route('employee.customerVisits', ['visit_plan' => $visit_plan]) }}">Visits</a>
                                            {{-- @endif --}}
                                            {{-- <a class="dropdown-item"
                                            ...
                                            @if (!$visit_plan->team_admin_approved_at && in_array($visit_plan->addedBy_id, $addeByArray))
                                                <a class="dropdown-item"
                                                    href="{{ route('employee.customerVisit.edit', ['type' => $visit_plan->type, 'visit' => $visit_plan]) }}">Edit</a>
                                            @endif
                                        </div>
                                ...
                                @if ($employee->team_admin)
                                    <td>
                                        @if ($visit_plan->employee_id == $employee->id)
                                            <span class="text-success">{{ $visit_plan->employee->name }}
                                                ({{ $visit_plan->employee->employee_id }})
                                            </span>
                                        @else
                                            {{ $visit_plan->employee->name }} ({{ $visit_plan->employee->employee_id }})
                                        @endif
                                ...
                                <td>
                                    ...
                                    @elseif ($employee->team_admin && !$visit_plan->team_admin_approved_at)
                                        <a href="{{ route('employee.customerVisitPlanStatusUpdate', ['status' => 'approved', 'type' => $visit_plan->type, 'visit' => $visit_plan]) }}"
