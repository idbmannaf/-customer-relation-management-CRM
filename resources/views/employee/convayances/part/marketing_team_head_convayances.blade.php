<div class="card">
    ...
    <div class="card-body" id="printArea">
        ...
        @if ($convayance->status != 'approved')
                ...
                <table class="table table-bordered table-sm text-nowarp">
                    ...
                    <tbody>
                        <tr class="@if(Agent::isMobile()) ifMobile @endif">
                            <td><input type="time" name="start_time" placeholder="Start Time" class="form-control start_time" id="start_time">
                            </td>
                            <td><input type="time" name="end_time" placeholder="End Time" class="form-control end_time" id="end_time">
                            </td>
                            <td><input type="text" name="start_from" placeholder="Start From" class="form-control start_from" id="start_from">
                            </td>
                            <td><input type="text" name="start_to" placeholder="To" class="form-control start_to" id="start_to">
                            </td>
                            <td>
                            ...
                            <td><button type="button"
                                    data-url="{{ route('employee.convayances.add', ['convayances' => $convayance, 'visit' => $visit]) }}"
                            ...
                            <tr class="@if(Agent::isMobile()) ifMobile @endif">
                                <td><input type="time" name="start_time" oninput="update(this,'start_time')"
                                    ...
                                    <input type="hidden" class="update_url"
                                        value="{{ route('employee.convayancesChangeAjax', ['convayances' => $convayance->id, 'item' => $item->id]) }}">
                                </td>
                                ...
                                <td>
                                    <button type="button" data-id="{{ $item->id }}"
                                        data-url="{{ route('employee.convayances.delete', ['convayances' => $convayance->id, 'item' => $item->id]) }}"
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <th colspan="2" class="total_amount">{{ $convayance->total_amount }}</th>
                        </tr>
                    </tfoot>
                    </tbody>
                </table>
            ...
            <div class="submit">
                <form
                    action="{{ route('employee.convayancesSubmit', ['convayances' => $convayance, 'visit' => $visit]) }}">
                    @if ($convayance->status == 'temp')
                        <input type="submit" name="submit" value="pending" class="btn btn-info">
                    @elseif ($convayance->status == 'pending')
                        <input type="submit" name="submit" value="rejected" class="btn btn-danger"
                            onclick="return confirm('Are you Sure? you want to Reject this convayances bill?')">

                        <input type="submit" name="submit" value="approved" class="btn btn-info"
                            onclick="return confirm('Are you Sure? you want to Approve this convayances bill?')">
                    @elseif ($convayance->status == 'rejected')
                        <input type="submit" name="submit" value="approved" class="btn btn-info"
                            onclick="return confirm('Are you Sure? you want to Approve this convayances bill?')">
                    @endif
