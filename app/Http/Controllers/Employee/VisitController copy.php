...
class VisitController extends Controller
{
    ...
    public function customerVisits(Request $request, VisitPlan $visit_plan)
    {
        ...
        return view('employee.visitPlan.visit.visit', compact('visit_plan', 'visits', 'offer', 'customer', 'customer_offer_items', 'brands'));
    }
    ...
    public function customerVisitCreate(Request $request, VisitPlan $visit_plan)
    {
        ...
        if ($visit) {
            ...
            // if (!$visit->approved_at && $visit->done_by) {
            //     return redirect()->back()->with('warning', 'You are Not Able to create More of This Visit');
            // } elseif (!$visit->approved_at || !$visit->done_by) {
            //     if (count($visit_plan->visits) > 0) {
            //         return redirect()->back()->with('warning', "You are not able to create visit plan until visit/Log approved and visit/log done. You have already {$visit_plan->visits->count()} visit");
            //     }
            // } else {
            //     $sibling_visit_id = $visit->id;
            // }
        ...
        // if (count($visit_plan->visits) > 0) {
        //     return redirect()->back()->with('warning', 'This Visit Plan Already have a Visit');
        // }
        ...
        // $spare_sparts_categories = ProductCategory::whereHas('products')->where('type','spare_sparts')->orderBy('name')->get();
        // $spare_sparts_categories = ProductCategory::whereHas('products')->get();
        $invoice = null;
                ...
                if ($visit_plan->invoice) {
                    ...
                    if ($visit->collection_amount < $total_invoice_amount) {
                        ...
                        if ($visit->next_paymnet_collection_date) {
                            ...
                            $collection_history->collection_approved_by = Auth::id();
                            $collection_history->prev_due = $customer->ledger_balance;
                            $collection_history->moved_amount = $visit->collection_amount - $due;
                            $collection_history->current_due = $collection_history->prev_due - $collection_history->moved_amount;
                            $collection_history->purpose = 'minus';
                            $collection_history->note = "{ $collection_history->moved_amount} Taka diducted from Customer Ledger Balance for Invoice ID ({$visit_plan->invoice_id})";
                            $collection_history->save();
...
die;
        ...
        $type = $request->type;
        menuSubmenu('visits', $type ??  'all' . "Visit");
        if ($request->sibling) {
        ...
        return view('employee.visitPlan.visit.allOfMyTeamMemberVisits', compact('visits', 'type'));
    }
