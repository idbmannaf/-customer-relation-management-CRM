    ...
    public function locationSet(Request $request)
    {
        ...
        if ($user->track) {
            if ($request->lat and $request->lng) {
                ...
                $location->data_by_app = $request->isApp ? 1 : 0;
                ...
                if ($location->lat and $location->lng) {
                    ...
                    if ($office_location) {
                        ...
                        if (Auth::user()->employee && $office_location->type == 'customer') {

                            $vistiPlanAttandances = VisitPlan::whereDate('date_time', Carbon::now())
                                ->orderBy('date_time')
                            ...
                            if ($vistiPlanAttandances) {
                                if ($vistiPlanAttandances->visit_start_at) {
                                    $vistiPlanAttandances->visit_end_at = now();
                                    $vistiPlanAttandances->save();
                                } else {
                                    $vistiPlanAttandances->visit_start_at = now();
                                    $vistiPlanAttandances->visit_end_at = now();
                                    $vistiPlanAttandances->save();
                                }
                            }
