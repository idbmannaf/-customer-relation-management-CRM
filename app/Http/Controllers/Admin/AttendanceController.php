...
class AttendanceController extends Controller
{
    ...
    public function attendeance(Request $request)
    {
        menuSubmenu('attendance', 'allAttendance');
        $status = $request->status;
        if ($status) {
            if ($status =='present') {
                $attendances = Attendance::with('company')
                ->whereDate('date', Carbon::now()->format('Y-m-d'))
                ->where('logged_in_at','!=',null)
                ->paginate(100);
            }else{
                $attendances = Attendance::with('company')
                ->whereDate('date', Carbon::now()->format('Y-m-d'))
        ...
        // })->orderBy('company_id')->paginate(50);
        return view('admin.attendance.index', compact('attendances','status'));
    }
    ...
    public function companyWiseAttendanceReport(Request $request)
    {
        menuSubmenu('attendance', 'companyWiseAttendanceReport');
        ...
        if ($request->type == 'filter') {
            ...
            //    $companyOffices = OfficeLocation::where('company_id',$request->company)->pluck('id')->toArray();
            $employees = Employee::whereHas('user', function ($q) {
                $q->where('office_location_id', '!=', null);
            })->where('company_id', $request->company)
                ->paginate(100);

            return view('admin.attendance.companyWiseAttandanceReport', [
                'input' => $request->all(),
                'companies' => $companies,
                'employees' => $employees,
            ]);
